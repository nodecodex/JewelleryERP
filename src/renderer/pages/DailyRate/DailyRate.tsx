...
export default function DailyRateView() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f3f4f6] text-foreground p-2 font-sans select-none no-print overflow-y-auto text-xs">
      
      {/* Banner Strip */}
      <div className="bg-orange-500 text-white py-1 px-4 text-center font-bold text-sm tracking-wider uppercase rounded-sm flex justify-between items-center shadow-inner mb-2 bg-gradient-to-r from-orange-500 via-amber-500 to-orange-500">
      ...
      <div className="surface-premium bg-card p-4 shrink-0 flex flex-col gap-4 mb-2">
        {/* Baseline rates inputs */}
        <div className="grid grid-cols-12 gap-4 items-center">
          
          {/* Voucher Date */}
          <div className="col-span-4 flex items-center gap-2">
            <span className="text-[11px] font-bold text-muted-foreground uppercase w-20">Vch Date</span>
            <div className="relative flex-1 flex items-center gap-1">
              <input
                type="date"
                className="w-full h-8 border border-border rounded-sm text-xs font-bold text-foreground px-2 bg-secondary/20 focus:bg-card focus:ring-1 focus:ring-amber-500 focus:outline-none"
                value={vchDate}
              ...
              <button 
                ...
                className="h-8 px-2 bg-amber-500 hover:bg-amber-600 text-white font-extrabold text-[10px] uppercase rounded-sm transition-colors"
              >
                F1
              </button>
            </div>
          ...
          {/* Gold Rate 10gm */}
          <div className="col-span-4 flex items-center gap-2">
            <span className="text-[11px] font-bold text-muted-foreground uppercase w-24">Gold Rate 10gm</span>
            <input
              type="number"
              className="flex-1 h-8 border border-border rounded-sm font-data text-right font-bold text-foreground bg-secondary/20 focus:bg-card focus:ring-1 focus:ring-amber-500 focus:outline-none px-2.5 text-sm"
              value={goldRate}
          ...
          {/* Silver Rate 1Kg */}
          <div className="col-span-4 flex items-center gap-2">
            <span className="text-[11px] font-bold text-muted-foreground uppercase w-24">Silver Rate 1Kg</span>
            <input
              type="number"
              className="flex-1 h-8 border border-border rounded-sm font-data text-right font-bold text-foreground bg-secondary/20 focus:bg-card focus:ring-1 focus:ring-amber-500 focus:outline-none px-2.5 text-sm"
              value={silverRate}
      ...
      <div className="surface-premium bg-card flex-1 overflow-auto mb-2 flex flex-col min-h-[300px]">
        <div className="flex-1 overflow-y-auto">
          <table className="w-full border-collapse text-left text-xs table-fixed min-w-[850px]">
            <thead>
              <tr className="sticky top-0 z-20 bg-secondary/50 border-b border-border text-[10px] font-bold text-muted-foreground uppercase select-none">
                <th className="w-[40px] border-r border-border px-2 py-2 text-center">Sr</th>
                <th className="w-[90px] border-r border-border px-2 py-2">GrCode</th>
                <th className="border-r border-border px-3 py-2 text-left">GrName</th>
                <th className="w-[80px] border-r border-border px-2 py-2 text-center">Metal</th>
                <th className="w-[85px] border-r border-border px-2 py-2 text-right">Touch</th>
                <th className="w-[75px] border-r border-border px-2 py-2 text-right">Tax%</th>
                <th className="w-[110px] border-r border-border px-2 py-2 text-right bg-secondary/20/50">Rate10gm</th>
                <th className="w-[110px] border-r border-border px-2 py-2 text-right bg-amber-50/20">TaxRate</th>
                <th className="w-[85px] border-r border-border px-2 py-2 text-right text-muted-foreground">PrTouch</th>
                <th className="w-[100px] border-r border-border px-2 py-2 text-right text-muted-foreground">PrRate</th>
            ...
            <tbody className="font-semibold text-foreground font-data select-text">
              {groupRows.map((row, index) => (
                <tr 
                  key={index}
                  className="border-b border-border hover:bg-secondary/20/60 transition-colors h-[32px] items-center"
                >
                  <td className="text-center font-bold text-muted-foreground border-r border-border">{row.sr}</td>
                  
                  {/* GrCode */}
                  <td className="p-0.5 border-r border-border">
                    <input
                      type="text"
                      className="w-full h-full border border-transparent rounded-sm hover:border-border focus:border-amber-500 focus:bg-card focus:outline-none px-1.5 font-bold uppercase text-[11px]"
                      value={row.grCode}
                  ...
                  <td className="p-0.5 border-r border-border">
                    <input
                      type="text"
                      className="w-full h-full border border-transparent rounded-sm hover:border-border focus:border-amber-500 focus:bg-card focus:outline-none px-2 text-[11px]"
                      value={row.grName}
                  ...
                  <td className="p-0.5 border-r border-border text-center">
                    <select
                      className="w-full h-full border border-transparent rounded-sm hover:border-border focus:border-amber-500 focus:bg-card focus:outline-none text-center font-bold text-foreground bg-transparent cursor-pointer"
                  ...
                  <td className="p-0.5 border-r border-border">
                    <input
                      type="number"
                      step="0.01"
                      className="w-full h-full border border-transparent rounded-sm hover:border-border focus:border-amber-500 focus:bg-card focus:outline-none text-right font-data px-1.5"
                      value={row.touch}
                  ...
                  {/* Rate10gm */}
                  <td className="px-3 text-right font-bold border-r border-border text-foreground text-[11.5px] bg-secondary/20/20 font-data">
                    ₹{row.rate10gm.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </td>

                  {/* TaxRate */}
                  <td className="px-3 text-right font-bold border-r border-border text-amber-600 text-[11.5px] bg-amber-50/5 font-data">
                    ₹{row.taxRate.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </td>

                  {/* PrTouch */}
                  <td className="px-2 text-right border-r border-border text-muted-foreground font-data">
                  ...
                  <td className="text-center p-0.5">
                    <button 
                      ...
                      className="p-1 hover:bg-rose-50 text-muted-foreground hover:text-rose-600 rounded-sm transition-colors"
        ...
        <div className="bg-secondary/20 border-t border-border px-4 py-2 flex justify-start items-center">
          <button 
            onClick={addNewRow}
            className="flex items-center gap-1.5 px-3 py-1 bg-primary hover:bg-primary text-white font-bold text-[10px] uppercase rounded-sm transition-all shadow-sm tracking-wider cursor-pointer"
          >
      ...
      <div className="surface-premium bg-card p-4 shrink-0 flex items-center gap-2 mb-2">
        <span className="text-[11px] font-bold text-muted-foreground uppercase w-20">Employee</span>
        <div className="relative flex-1 max-w-sm flex items-center">
          <input
            type="text"
            placeholder="Name or ID"
            className="w-full h-8 border border-border rounded-sm px-2.5 font-bold text-foreground focus:ring-1 focus:ring-amber-500 focus:outline-none text-xs"
            value={employee}
      ...
      <div className="surface-premium bg-card px-4 py-2.5 flex justify-end items-center gap-2 shrink-0 shadow-sm select-none">
        
        {/* Save button */}
        <button 
          onClick={handleSave}
          disabled={isSaving}
          className="flex items-center gap-1.5 px-4 h-8 bg-amber-500 hover:bg-amber-600 text-white border border-amber-600 text-xs font-bold rounded-sm transition-all shadow-sm uppercase tracking-wider cursor-pointer active:scale-95 disabled:opacity-50"
        >
        ...
        <button 
          onClick={handlePrint}
          className="flex items-center gap-1.5 px-4 h-8 bg-secondary/50 hover:bg-secondary text-foreground border border-border text-xs font-bold rounded-sm transition-all shadow-sm uppercase tracking-wider cursor-pointer active:scale-95"
        >
        ...
        <button 
          ...
          className="flex items-center gap-1.5 px-4 h-8 bg-secondary/50 hover:bg-secondary text-foreground border border-border text-xs font-bold rounded-sm transition-all shadow-sm uppercase tracking-wider cursor-pointer active:scale-95"
        >
          <Undo2 className="h-4 w-4 text-muted-foreground" /> Cancel
        </button>

        {/* Delete button */}
        <button 
          onClick={handleDelete}
          className="flex items-center gap-1.5 px-4 h-8 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 hover:border-rose-300 text-xs font-bold rounded-sm transition-all shadow-sm uppercase tracking-wider cursor-pointer active:scale-95"
        >
        ...
        <button 
          onClick={handleExit}
          className="flex items-center gap-1.5 px-4 h-8 bg-primary hover:bg-primary text-white border border-border text-xs font-bold rounded-sm transition-all shadow-sm uppercase tracking-wider cursor-pointer active:scale-95"
        >
