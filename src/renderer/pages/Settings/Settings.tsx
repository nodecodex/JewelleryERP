    ...
    <div className="p-3 bg-background h-full overflow-y-auto  font-sans">
      <div className="flex justify-between items-center border-b border-border pb-2 mb-3">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground font-semibold uppercase">Manage global daily metal rates, local database backups, and cloud synchronization.</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-3">
        <button
          ...
          className={`h-7 px-4 font-bold text-xs uppercase rounded-sm border transition-all ${
            activeSettingsTab === 'general'
              ? 'bg-primary border-border text-white shadow-sm'
              : 'bg-card border-border text-muted-foreground hover:bg-secondary/20'
          }`}
        >
      ...
      {activeSettingsTab === 'general' ? (
        ...
        <div className="col-span-6 space-y-3">
          <div className="bg-card border border-slate-355 rounded-sm shadow-sm overflow-hidden h-fit">
                ...
                <form onSubmit={handleSaveRates} className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        type="number"
                        className="erp-input font-data text-right font-bold text-foreground"
                  ...
                  <button
                    type="submit"
                    className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white font-extrabold rounded-sm text-xs uppercase border border-amber-600 transition-all shadow-sm"
                  >
                    Update Rates Table
                  </button>
                </form>
            ...
            <div className="p-3 space-y-3">
              <div>
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1 bg-primary hover:bg-primary text-white border border-border text-xs font-bold rounded-sm transition-all shadow-sm uppercase tracking-wider"
                >
              ...
              <form onSubmit={handleRestore} className="border-t border-border pt-3 space-y-2">
                ...
                <button
                  type="submit"
                  className="px-3 py-1.5 bg-rose-650 hover:bg-rose-700 text-white font-bold rounded-sm text-xs uppercase border border-rose-700 transition-colors shadow-sm"
                >
          ...
          <div className="bg-card border border-slate-355 rounded-sm shadow-sm overflow-hidden">
            ...
            <div className="p-3 space-y-3">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500 hover:bg-amber-600 disabled:bg-secondary border border-amber-600 text-white font-extrabold rounded-sm text-xs uppercase transition-all shadow-sm"
              >
                ...
                <div className={`p-2 rounded-sm text-xs border ${
                  ...
                  {syncStatus.success ? (
                    <div>
                      ...
                      {syncStatus.lastSyncTime && <p className="mt-0.5 text-[9px] text-muted-foreground font-data">Timestamp: {syncStatus.lastSyncTime}</p>}
                    </div>
                  ) : (
                    <span className="flex items-center gap-1">
                      <ShieldAlert className="h-3.5 w-3.5" />
